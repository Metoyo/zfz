    ...
    .controller('ZujuanCtrl', ['$rootScope', '$scope', '$location', '$http', 'urlRedirect', '$q', '$timeout',
      ...
      function ($rootScope, $scope, $location, $http, urlRedirect, $q, $timeout, DataService, $cookieStore) {
        ...
        $scope.deleteDaTi = function(tx){
          if(confirm('删除大题会导致其下题目丢失，是否确认删除？')){
            $scope.sjzSet['组卷规则'] = Lazy($scope.sjzSet['组卷规则']).reject({'题型ID': tx['题型ID']}).toArray();
         ...
         * 由是试卷列表点击展示试卷详情
         */
        $scope.showPaperDetail = function(sjzDt){
          var obj = {
            method: 'GET',
            url: shiJuanZuUrl,
            params: {
            ...
            if(data.result && data.data){
              ...
              Lazy($scope.sjzSet['组卷规则']).each(function(dt){
                ...
                if(sjrArr && sjrArr.length > 0){
                  Lazy(sjrArr).each(function(sjr){
                    ...
                    sjr['难度名称'] = ndName;
                    //sjr['知识点名称'] = zsdName;
                    sjzZsdArr = Lazy(sjzZsdArr).union(sjr['知识点']).toArray();
                  });
                }
              ...
              if(sjzZsdArr.length > 0){
                var zsdObj = {
                  ...
                  url: zhiShiDianUrl,
                  params: {
                    '知识点ID': JSON.stringify(sjzZsdArr)
                  }
                };
                $http(zsdObj).success(function(zsdData){
                  if(zsdData.result && zsdData.data.length > 0){
                    Lazy($scope.sjzSet['组卷规则']).each(function(dt){
                      if(dt['随机题目'] && dt['随机题目'].length > 0){
                        Lazy(dt['随机题目']).each(function(sjr){
                          var zsdName = [];
                          Lazy(sjr['知识点']).each(function(zsdId){
                            var fidZsd = Lazy(zsdData.data).find(function(zsd){ return zsd['知识点ID'] == zsdId});
                            if(fidZsd){
                              zsdName.push(fidZsd['知识点名称']);
                            }
                          });
                          sjr['知识点名称'] = zsdName;
                        });
        ...
        $scope.addSingleSj = function(){
          ...
          if($scope.zuJuanParam.sjScore == $scope.sglSjParam.sjScore){
            var obj = {
              method: 'PUT',
              url: shiJuanZuShiJuanUrl,
              data: {
            ...
            DataService.alertInfFun('pmt', '已选题目总分和试卷组总分不一致，请修改！');
          }
        };
