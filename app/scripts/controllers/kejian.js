    ...
    angular.module('zhifzApp.controllers.KejianCtrl', []) //controller 开始
      .controller('KejianCtrl', ['$rootScope', '$scope', '$http', '$location', '$timeout', 'DataService', '$cookieStore',
        function ($rootScope, $scope, $http, $location, $timeout, DataService, $cookieStore) { // 002 开始
          ...
          var yongHuCeYanUrl= '/yonghu_ceyan'; //查询用户测验
          var exportStuUrl = '/json2excel'; //导出考生
          var keXuHaoCeYanUrl = '/kexuhao_ceyan'; //课序号测验
          ...
          $scope.classTestDetail = function(cy){
            $scope.classTestDtl = cy;
            var tiMuIds = [];
            Lazy(cy['测验设置']['组卷规则']).each(function (dt) {
              Lazy(dt['固定题目']).each(function(tm){
                if(tm['题目ID']){
                  tiMuIds.push(tm['题目ID']);
                }
              });
            });
            if(tiMuIds.length){
              ...
              DataService.alertInfFun('err', '次测验没有题目数据！');
            }
          };
          // $scope.classTestDetail = function(cy){
          //     var id = cy['测验ID'];
           ...
           * 保存题目
           */
          $scope.saveTiMu = function(fromImg, tp){
            var mis = [];
            var tmId = '';
            ...
            $http(obj).success(function(data){
              if(data.result){
                ...
                if(tp == 'list'){
                  $scope.classTestDetail($scope.classTestDtl);
                  $scope.closeAddTiMuPop();
                }
           ...
           * 重现发布测验
           */
          $scope.rePublishCeYan = function(){
            if($scope.classTestDtl){
              var obj = {
                method: 'POST',
                url: ceYanUrl,
                data: {
                  '测验ID': $scope.classTestDtl['测验ID'],
                  '测验设置': JSON.stringify($scope.classTestDtl['测验设置'])
                }
              };
              $scope.loadingImgShow = true;
              $http(obj).success(function(pData){
                if(pData.result){
                  DataService.alertInfFun('suc', '测验发布成功！');
                }
                else{
                  DataService.alertInfFun('err', pData.error);
                }
              ...
              DataService.alertInfFun('err', '请选择要重现发布的测验！');
            }
          };
          ...
          $scope.getTongJiDt = function(id, tp){
              ...
              $http(objUid).success(function(data){
                if(data.result && data.data){
                  ...
                  Lazy(disByUid).each(function(v, k, l){
                    var newObj = {
                      '总测验次数': $scope.ceYanTongJiDt.testNum,
                      '参与测验次数': v.length || 0,
                      '参与率': '0%',
                      '总题数': $scope.ceYanTongJiDt.itemNum,
                      '正确题数': 0,
                      '正确率': '0%'
                    };
                    if(v && v.length > 0){
                    ...
                    newObj['参与率'] = ((newObj['参与测验次数']/(newObj['总测验次数'] || 1)) * 100).toFixed(1) + '%';
                    newObj['正确率'] = ((newObj['正确题数']/(newObj['总题数'] || 1)) * 100).toFixed(1) + '%';
            ...
            if(id && tp == 'kxh'){
              ...
              if(findTar){
                ...
                $http(objKxhCy).success(function(data){
                  if(data.result && data.data.length > 0){
                    $scope.ceYanTongJiDt.testNum = data.data.length;
                    Lazy(data.data).each(function (tt) {
                      $scope.ceYanTongJiDt.itemNum += tt['题目数'];
                    });
                  }
                  else{
                    DataService.alertInfFun('err', data.error);
          ...
          $scope.exportTestScore = function () {
            ...
            Lazy(exportStu).each(function(ks){
              ...
              if(ks['测验成绩']){
                ...
                ksObj['参与率'] = ks['测验成绩']['参与率'];
                ksObj['总题数'] = ks['测验成绩']['总题数'];
                ksObj['正确题数'] = ks['测验成绩']['正确题数'];
                ksObj['正确率'] = ks['测验成绩']['正确率'];
              }
              else{
                ksObj['总测验次数'] = $scope.ceYanTongJiDt.testNum;
                ksObj['参与测验次数'] = 0;
                ksObj['参与率'] = '0%';
                ksObj['总题数'] = $scope.ceYanTongJiDt.itemNum;
                ksObj['正确题数'] = 0;
                ksObj['正确率'] = '0%';
              }
              ksArr.push(ksObj);
